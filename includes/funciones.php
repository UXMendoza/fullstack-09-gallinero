...
function mostrarProduccionSemana()
{
    ...
    $query = "SELECT gallina.nombre, gallina.nombre_raza, SUM(produccion.cantidad) as total FROM gallina JOIN produccion ON gallina.id = produccion.id_gallina WHERE produccion.fecha_recogida BETWEEN DATE_SUB(curdate(),INTERVAL 1 WEEK) AND curdate() GROUP BY gallina.nombre, gallina.nombre_raza ORDER BY total DESC;";
...
function mostrarMediaRazaMes()
{
    ...
    $query = "SELECT gallina.nombre_raza, ROUND(AVG(produccion.cantidad), 2) AS total FROM produccion JOIN gallina ON produccion.id_gallina = gallina.id WHERE produccion.fecha_recogida BETWEEN DATE_SUB(curdate(),INTERVAL 1 MONTH) AND curdate() GROUP BY gallina.nombre_raza ORDER BY total DESC;";
...
function mostrarDiasMasProductivos()
{
    include("db_connection.php");
    ...
    $query = "SELECT fecha_recogida, SUM(cantidad) AS huevos FROM produccion GROUP BY fecha_recogida ORDER BY `huevos` DESC LIMIT 5;";
...
function mostrarMosEncimaMedia()
{
    ...
    $query = "SELECT gallina.nombre, ROUND(AVG(produccion.cantidad), 2) FROM gallina JOIN produccion ON gallina.id = produccion.id_gallina WHERE produccion.cantidad > (SELECT AVG(produccion.cantidad) FROM produccion) AND gallina.nombre_raza = 'Gallina de Mos' GROUP BY gallina.nombre ORDER BY ROUND(AVG(produccion.cantidad), 2) DESC;";
...
function mostrarPromedio()
{
    ...
    $query = "SELECT gallina.nombre, ROUND(AVG(produccion.cantidad), 2) AS media FROM gallina JOIN produccion ON produccion.id_gallina = gallina.id GROUP BY gallina.nombre ORDER BY media DESC;";
...
function mostrarGallinasMasProductivas()
{
    include("db_connection.php");
